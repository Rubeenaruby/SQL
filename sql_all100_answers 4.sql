...
(3, 3, '2023-05-22 08:54:00', '2023-06-17 00:00:00');


select round(avg(extract(epoch from(resolved_at::timestamp-created_at::timestamp))/3600),2)
from support_tickets;

--  SELECT Round(avg(abs(timestampdiff( hour,resolved_at ,created_at))),2) as average_response_time
-- from support_tickets
...
select c.id,c.name,c.city, floor(sum(o.amount)) as total_spending
from customers c
join orders o
on c.id=o.customer_id
where o.amount =(
select max(o1.amount)
from customers c1
join orders o1
...
(1, 'salgate@f2.com'),
(2, 'lwycliff@list-manage.com');


select p.name , count(r.product_id)
from products p
join requests r
on p.id=r.product_id
where is_available=1
group by p.name
order by count(r.product_id) desc, p.name;


SELECT
    p.name AS product_name,
    COALESCE(COUNT(r.product_id), 0) AS total_requests
FROM products p
...
select c.name, count(e.campaign_id) as total_engagements, sum(e.views + e.clicks) as total_views_clicks
from campaigns c
join engagements e
...
select a.email, round(sum(r.amount),2)
from accounts a
join reports r
on a.id=r.account_id
where substr(dt,1,4)='2023'
--where year(r.dt)=2023
group by a.email
order by a.email;
...
-- select d.mac_address, count(s.filename) as total_scanned, sum(s.is_infected) as total_infected_files
-- from devices d
...
SELECT d.mac_address,  COUNT(s.filename) AS total_file, 
SUM(CASE WHEN s.is_infected = 1 THEN 1 ELSE 0 END) AS infected
FROM devices d
...
select c.name,sum(t.amount) as total_amount,count(t.dt) as total_transactions
from coins c
join transactions t
on  c.id=t.coin_id
where substr(t.dt,1,4)='2023'
group by c.name
order by c.name;
...
select c.email, count(d.name) as domain_name
from customers1 c
join domains d
...
select p.name,p.price,count(w.product_id) as total_wishlist
from products1 p
join wishlists w
...
select l.name,max(o.amount) as highest_offer,count(o.amount) as total_offer
from lots l
join offers o
on l.id=o.lot_id
group by l.name
...
select a.iban,min(t.amount),max(t.amount),round(avg(t.amount),2),count(t.amount)
from accounts1 a
join transactions1 t
on a.id=t.account_id
where substr(t.dt,1,7)='2022-09'
group by a.iban
order by a.iban
...
select p.name,p.price,count(w.product_id) as total_wishes
from products2 p
join wishlists2 w 
on p.id=w.product_id
where in_stock=1
...
select c.email,count(p.customer_id) as total_purchases, round(sum(p.amount),2) as total_purchase_amount
from customers2 c
join purchases2 p
on c.id=p.customer_id
where substr(dt,1,7)='2024-03'
group by c.email
order by c.email
...
SELECT a.email,  ap.dt AS scheduled_appointment, (DATE '2024-04-10' - ap.dt::date) AS days_of_delay
FROM applicants a
JOIN appointments ap 
ON a.id = ap.applicant_id
WHERE is_received='0' and ap.dt < '2024-04-10'
...
SELECT a.email,  TO_CHAR(TO_DATE(ap.dt, 'YYYY-MM-DD'), 'Day') AS scheduled_appointment
FROM applicants2 a
JOIN  appointments2 ap 
ON a.id = ap.applicant_id
WHERE EXTRACT(DOW FROM TO_DATE(ap.dt, 'YYYY-MM-DD')) IN (0, 6) 
...
select c.name , count(d.is_Active)
from countries2 c
join domains3 d
...
select d.name,'2024-04-10' as today_date, 
(d.next_renewal_date::date - '2024-04-10'::date) AS days_until_renewal
from domains4 d
order by days_until_renewal, d.name;

select d.name,'2024-04-10' as today_date, next_renewal_date::date,
(d.next_renewal_date::date - DATE '2024-04-10') AS days_until_renewal
from domains4 d
order by days_until_renewal, d.name;

--DATE is used with string literal and  ::date is used with column refernce
--::int , ::text, ::date, ::timestamp
...
select u.email, count(t.amount),round(min(t.amount),2),round(max(t.amount),2), round(sum(t.amount),2)
from users u
...
select u.email, count(t.amount), round(sum(t.amount),2)
from users2 u
join transactions4 t
on u.id=t.user_id
where substr(t.dt,1,4)='2023'
group by u.email
order by u.email
...
select c.name,round(avg(t.amount),2) as total
from coins1 c
join transactions5 t
on c.id=t.coin_id
where EXTRACT(YEAR FROM t.dt::date)= 2023
group by c.name
order by total limit 3

--we can extract year month day, hour
-- --------------------------------question 23-----------------------------------------
CREATE TABLE coin (
    id INT PRIMARY KEY,
    name VARCHAR(255)
...
select c.name, count(t.amount) as total, round(min(t.amount),2),round(max(t.amount),2),round(avg(t.amount),2)
from coin c
join transaction t
on c.id=t.coin_id
where 
  EXTRACT(YEAR FROM t.dt::date) = 2024
...
select extensions, count(is_suspicious) as total
from suspicious_files
where is_suspicious=1 and EXTRACT(year from scan_dt::date)=2024 and EXTRACT(month from scan_dt::date)=3
group by extensions
...
select c.email,count(is_scanned)as total
from clients c
join device d
on c.id= d.client_id
where substr( schedule_scan_dt,1,7)='2024-03' and is_scanned=1
group by c.email
order by c.email
-- -----------------------------------------question 26------------------------------------------------------
...
select c.email, round(avg(s.cpu_usage),2) as avg_cpu ,round(avg(s.disk_usage),2) as avg_disk,round(avg(s.memory_usage),2) as avg_memory
from customers3 c
join site_metrics s
on c.id=s.customer_id
group by c.email
having avg(s.cpu_usage) > 50
...
select c.email,count(s.is_active)
from customers4 c
join sites s
on c.id= s.customer_id
where s.is_active=1
...
select a.iban, round(avg(i.amount),2) as avg_income,round(sum(i.amount),2)
from account a 
join income1 i
on a.id=i.account_id
where (i.dt::date) between DATE '2024-01-01'  AND DATE '2024-03-31'
group by a.iban
order by  avg_income desc ,a.iban  limit 3
...
select a.iban, round(sum(i.amount),2)  as total_income, '20%' as rate, round(sum(i.amount) *0.2,2) as cal_tax
from account01 a
join income01 i
on a.id=i.account_id
where substr(i.dt,1,4)='2023'
group by a.iban
order by a.iban
...
INSERT INTO customers0 (id, email) VALUES
(1, 'otoohey0@elpais.com'),
(2, 'egrebbin1@state.gov'),
(3, 'arides2@sohu.com');

INSERT INTO expenses0 (customer_id, dt, amount) VALUES
...
INSERT INTO incomes0 (customer_id, dt, amount) VALUES
...
select c.email,total_expense,total_income
from customers0 c
join(
...
INSERT INTO cust (id, email) VALUES
(1, 'dtollmache0@typepad.com'),
(2, 'eclutterbuck1@baidu.com'),
(3, 'mdensun2@ustream.tv');

INSERT INTO expe (customer_id, amount) VALUES
(1, 136.18),
(1, 323.28),
(1, 383.37),
(1, 505.41),
...
INSERT INTO inc (customer_id, amount) VALUES
(1, 39.44),
(1, 49.49),
(1, 292.19),
(1, 419.36),
...
select c.email, round(total_income-total_expense,2) as balance 
from cust c
join(
	select customer_id, sum(amount) as total_expense
	from expe 
...
where round(total_income-total_expense,2)< 0
order by c.email
...
select p.name , to_char(s.dt::date , 'Month'), round(sum(s.amount),2) as total_sales
from product p
join sale s
on p.id=s.product_id
where s.dt::date>= DATE '2024-01-01' and s.dt::date< '2024-04-01'
group by p.name, to_char(s.dt::date , 'Month'), extract(month from s.dt::date)
...
select p.name, count(e.ein) as emp_count, ceil(avg(experience_years)) as avg_experience ,
case when count(e.ein)<  5 then 'yes' else 'no' end as is_understaffed
from projects_employees pe
join projects p on pe.project_id=p.id
join employees e on pe.employee_id=e.id
group by p.name
having avg(experience_years) > 2
order by emp_count desc , p.name
...
INSERT INTO transactions6 (dt, wallet, amount) VALUES
...
drop table transactions6
select 
t.wallet, count(*)  as toal_transactions,
ROUND(SUM(CASE WHEN t.amount>0 THEN t.amount ELSE 0 END ),2) as total_bought,
ROUND(abs(SUM(CASE WHEN t.amount<0 then t.amount else 0 end )),2) as total_sold
from transactions6 t
where substr(t.dt,1,7)='2024-02'
group by t.wallet
order by t.wallet
...
CREATE TABLE leave_records (
    ...
    leave_dt VARCHAR(19),
    days_taken INT,
    FOREIGN KEY (employee_id) REFERENCES employees11(id)
);

INSERT INTO employees11 (id, email) VALUES
...
select e.email, sum(days_taken),
CASE WHEN sum(days_taken)<=20 then 'within limit' else 'exceeded' END AS leave_status
from employees11 e
join leave_records l
on e.id=l.employee_id
where  substr(l.leave_dt,1,4)='2023'
group by e.email
order by e.email
------------------------------question 36--------------------------------------------------------
...
sum(emails_sent),
sum(emails_opened),
round(sum(emails_opened)*1.0/sum(emails_sent)*100,2) as open_rate
from campaign c
join email_stat e
on c.id=e.campaign_id
group by c.name
having (sum(emails_opened)*1.0/sum(emails_sent))*100 > 50
...
select b.name, count(m.maturity), min(m.maturity) as earliest_maturity,
    max(m.maturity) as latest_maturity, ceil(avg(m.maturity-DATE '2023-09-13'))
from bonds b
join maturities m
on b.id=m.bond_id
group by b.name
having avg(m.maturity-DATE '2023-09-13')>365
order by b.name
------------------------------question 38--------------------------------------------------------
CREATE TABLE bond(
    id INT PRIMARY KEY,
...
select  b.name, count(i.rate), min(i.rate), max(i.rate), round(avg(i.rate),2)
from bond b
...
select bh.name, sum(b.annual_coupon * b.coupons_remaining)  as total_cash_flow
from bondholders_bonds bb
join bondholders bh on bb.bondholder_id=bh.id
join bonds0 b on bb.bond_id=b.id
group by bh.name
...
select i.email,count(*), min(c.cash_flow), max(c.cash_flow), round(avg(c.cash_flow),2)
from investors i
join cash_flows c
...
select i.email, count(*), sum(c.expected_flow), (max(c.expected_flow)-min(c.expected_flow)) as range
from investor i
join cash_flow c
on i.id=c.investor_id
group by i.email
...
select c.coupon_code, c.description,count(is_enabled),min(u.amount),max(u.amount), round(avg(u.amount),2)
from coupons c
join coupon_uses u
on c.id=u.coupon_id
where is_enabled=1
...
INSERT INTO project (id, freelancer_id, status, income) VALUES
...
count(distinct f.id) as total_freelancer,
round(avg(p.income),2)
from freelancers f
 join professions pf on pf.id=f.profession_id
join project p on f.id=p.freelancer_id
...
INSERT INTO product1 (id, category_id, title, sku, stock_number) VALUES
...
select c.title, p.title, sum(stock_number) as total
from categories c
join product1 p
on c.id=p.category_id
group by c.title, p.title
having sum(stock_number)>10
order by c.title,p.title, total desc
...
select q.domain_name,t.threat_type, count(q.domain_name), sum(q.users_affected) as  affect
from threat_types t
join quarantine_urls q
on t.id=q.threat_id
where q.status='Quarantined'
group by q.domain_name,t.threat_type
order by affect desc, q.domain_name
------------------------------question 46--------------------------------------------------------
...
select c.mac_address,count(*),sum(s.traffic) as total
from client0 c
join stream s
on c.id=s.client_id
...
select n.cidr,
count(*), 
concat(ceil(avg(cast(replace(i.cpu_usage, '%','') as float) )),'%'),
concat(ceil(avg(cast(replace(i.memory_usage, '%','') as float))),'%'),
concat(ceil(avg(cast(replace(i.network_usage,'%','') as float))),'%')
from networks n
join instances i
on n.id=i.network_id
where n.id in(
select distinct network_id from instances 
where cast(replace(cpu_usage,'%','') as float) >= 80
)
group by n.cidr
order by n.cidr;
...
select t.hash, floor(sum(extract(epoch from p.end_dt::timestamp - p.start_dt::timestamp))) as usage_time
from tasks t
join processes p
on t.id=p.task_id
group by t.hash
...
select 
case when d.score >= 80 then 'A' 
	when d.score >=60 and d.score<80 then 'B'
	when d.score>=40 and d.score<60 then 'C'
	when d.score>=20 and d.score<40 then 'D'
	else 'F'
end
from device0 d
order by d.id
...
(3, 1, 125),
(3, 2, 31);
select a.username,a.email, max(t.name),sum(r.amount),round(sum(t.cost* r.amount),2)
from readings r
join tariffs t on t.id=r.tariff_id
join accounts11 a on a.id=r.account_id
...
select a.username,a.email,count(*) as total_item,
sum(i.weight) as total_weight
from accounts_items ai
join acc a on a.id=ai.account_id
...
select c.name,count(*) as banners  ,min(b.width*b.height),ceil(avg(b.width*b.height)), max(b.width*b.height), sum(b.width* b.height)
from cities c
join banners b
on c.id=b.city_id
group by c.name
...
select l.name, count(o.lot_id) as offer , min(o.amount),round(avg(o.amount),2),max(o.amount)
from lot l
left join offer o
on l.id=o.lot_id
group by l.name
...
select concat(p.last_name,' ',p.first_name) as fullname,
p.email,
count(*) as relations,
count(CASE WHEN is_approved=TRUE then 'true' END) as approval, 
count(CASE WHEN is_approved=FALSE then 'false' END) as pending
from profiles p
join relations r
...
CREATE TABLE transactions7 (
    ...
    dt TIMESTAMP,
    amount DECIMAL(5,2),
    FOREIGN KEY (account_id) REFERENCES accounts5(id)
);

drop table transactions7
INSERT INTO accounts5 (id, iban) VALUES
(1, 'SE48 2961 2087 8112 2835 6438'),
(2, 'BE89 2286 5514 4847'),
(3, 'MU84 HRGV 2047 2584 5774 3195 [856] PZ');
INSERT INTO transactions7 (account_id, dt, amount) VALUES
...
select a.iban,count(t.amount), sum(t.amount) as total
from accounts5 a
...
select c.mac,sum(amount), round(sum(t.amount*c.tariff),2) as cost
from clients1 c
join traffic t
on c.id=t.client_id
where  substr(t.dt,1,7)='2022-05'
group by c.mac
order by cost desc
...
select c.name,c.address,c.phone, round(avg(cg.review_rating),2) asoverall_review
from companies c
join categorie  cg
on c.id=cg.company_id
group by c.name,c.address,c.phone
order by avg(review_rating) desc , c.name
...
select a.username,count(*) as domains, min(d.expiration_date)
from accounts10  a
join domains10 d
on a.id=d.account_id
where is_active=1 and d.expiration_date > '2022-07-15'
group by a.username
order by a.username;
------------------------------question 60--------------------------------------------------------
CREATE TABLE camp (
...
SELECT c.name AS campaign, COUNT(*) AS events, ROUND(AVG(e.value), 5) AS average_value
FROM  camp c
...
select p.first_name,p.last_name,p.email,s.job_success_score
from profiles1 p
join stats s
on p.id=s.profile_id
where is_verified=1 AND  s.job_success_score>=90
...
GROUP BY c.name
ORDER BY deployments DESC, configuration  desc;
...
from events1 e
where e.type='buy' and e.dt like '2022-05%'
...
select c.name, sum(cg.revenue-cg.expenses) as profit
from companie c
join campaign1 cg
on c.id=cg.company_id
group by c.name
having sum(cg.revenue-cg.expenses)>0
...
select c.mac,c.upstream_rate, c.downstream_rate, c.downtime_rate
from clients11 c
where c.downstream_rate>c.upstream_rate and c.downtime_rate='never' or c.downtime_rate='once'
order by c.mac
------------------------------question 67--------------------------------------------------------
CREATE TABLE owners (
    id SMALLINT PRIMARY KEY,
...
select o.full_name,o.email_address,e.dt,e.title
from owners o
join eve e
on o.id=e.owner_id
where on_vacation=0
order by e.dt limit 5
------------------------------question 68--------------------------------------------------------
...
INSERT INTO trans (wallet_id, credit) VALUES
(3, 5.21),
(6, 24.04),
(8, 29.66),
(10, 1.67),
...
INSERT INTO customers12 (id, customer_name) VALUES
(401, 'Hubert Keesler'),
(402, 'Devin Vert'),
...
INSERT INTO orders12 (order_id, customer_id, order_type, order_amount) VALUES
...
SELECT c.customer_name, ROUND(SUM( CASE 
     WHEN o.order_type = 'Buy' THEN o.order_amount * 0.001
     WHEN o.order_type = 'Sell' THEN o.order_amount * 0.0015
  ELSE 0 END ), 2) AS total_fees
FROM customers12 c
JOIN orders12 o ON c.id = o.customer_id
...
round(sum(t.amount* (1+c.interest_rate/100.0)),2) as amount
...
--  some SQL engines might perform integer division, which would result in 0 instead of 0.15.
-- But if one operand is a float (e.g., 15 / 100.0), the result is a decimal: 0.15.
...
INSERT INTO ACCOUNTS12 (id, account_holder, amount) VALUES
...
-- select year , month, total_transactions
-- from
-- where
-- group by
-- having
...
INSERT INTO COUNTR (ID, NAME, MIN_SIZE) VALUES
...
INSERT INTO CUSTOMERS11 (id, name, phone, visited_on, amount) VALUES
...
CREATE TABLE SALARY (
    EMPLOYEE_ID INT CHECK (EMPLOYEE_ID BETWEEN 1 AND 1000),
    COMPANY_ID INT CHECK (COMPANY_ID BETWEEN 1 AND 1000),
...
CREATE TABLE SCHEDULE0 (
    PROFESSOR_ID INT,
    COURSE_ID INT,
    SEMESTER INT,
    YEAR INT,
    FOREIGN KEY (PROFESSOR_ID) REFERENCES PROFESSORS(ID),
    FOREIGN KEY (COURSE_ID) REFERENCES COURSE42(ID)
);
DROP TABLE DEPT
DROP TABLE PROFESSORS
DROP TABLE COURSE42
DROP TABLE SCHEDULE0
...
INSERT INTO COURSE42 (ID, NAME, DEPARTMENT_ID, CREDITS) VALUES
(9, 'Clinical Biochemistry', 2, 3),
(4, 'Astronomy', 1, 6),
(10, 'Clinical Neuroscience', 2, 5),
(1, 'Pure Mathematics and Mathematical Statistics', 1, 3),
...
INSERT INTO SCHEDULE0 (PROFESSOR_ID, COURSE_ID, SEMESTER, YEAR) VALUES
(5, 3, 6, 2012),
(7, 3, 1, 2013),
(5, 7, 6, 2010),
(2, 10, 2, 2004),
...
CREATE TABLE PROFESSOR00 (
    ID INT PRIMARY KEY CHECK (ID BETWEEN 1 AND 1000),
    NAME VARCHAR(100) NOT NULL CHECK (LENGTH(NAME) BETWEEN 1 AND 100),
    DEPARTMENT_ID INT,
    SALARY INT CHECK (SALARY BETWEEN 5000 AND 40000),
    FOREIGN KEY (DEPARTMENT_ID) REFERENCES DEPARTMENT00(ID)
);

INSERT INTO DEPARTMENT00 (ID, NAME) VALUES
(3, 'Biological Sciences'),
(5, 'Technology'),
(6, 'Humanities & Social Sciences'),
(2, 'Clinical Medicine'),
(4, 'Arts and Humanities'),
(1, 'Physical Sciences');
INSERT INTO PROFESSOR00 (ID,NAME, DEPARTMENT_ID, SALARY) VALUES
...
CREATE TABLE REGISTER (
    STUDENT_ID INT,
    MAJOR_ID INT,
    FOREIGN KEY (STUDENT_ID) REFERENCES STUDS(STUDENT_ID),
    FOREIGN KEY (MAJOR_ID) REFERENCES MAJORS(MAJOR_ID)
);

INSERT INTO STUDS (STUDENT_ID, STUDENT_NAME, STUDENT_AGE) VALUES
(1, 'John', 20),
(2, 'Masie', 21),
(3, 'Harry', 21);
...
INSERT INTO STUD (ID, AGE, SCORE) VALUES
(1, 19, 91),
(2, 20, 90),
(3, 20, 87),
(4, 21, 72),
...
INSERT INTO EMPLOYE(NAME, PHONE, AGE) VALUES
('Sam', '1000040000', 30),
('Alex', '1000020000', 60),
('Alex', '1000020012', 65),
('Sam', '1000040000', 30),
...
INSERT INTO transactions11 (dt, customer, debit, credit) VALUES
...
('2021-11-13 10:32:54', 'Rasla Venny', 45.42, 55.81),
('2021-12-28 00:00:00', 'Rasla Venny', 68.17, 85.30);
